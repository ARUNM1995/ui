...
export class MenuService {
  ...
  public getVerticalMenuItems():Array<Menu> {
   ...
   return this.prepareMenu(verticalMenuItems);
    //return verticalMenuItems;
  }

  public getHorizontalMenuItems():Array<Menu> {
    return this.prepareMenu(horizontalMenuItems);
    //return horizontalMenuItems;
  }

  prepareMenu(menus:Array<Menu>){
    let roles=this.authService.getUserRoles();
   
/*
    if(roles[0]=='ADMIN'){
         menus[2].isAdmin=false;
         menus[3].isAdmin=false;
         menus[4].isAdmin=false;
         menus[5].isAdmin=false;
       }*/
       roles.forEach(role=>{
          if(role=="ADMIN"){
            menus[2].isAdmin=false;
  ...
  public createVerticalMenuItem(menu:Array<Menu>, menuItem){
   ...
   // this.renderer2.setAttribute(div, 'hidden',menuItem.isAdmin);
    this.renderer2.setAttribute(div, 'id', 'menu'+menuItem.id);
    this.renderer2.setProperty(div, 'hidden',menuItem.isAdmin);
    this.renderer2.setAttribute(div, 'tourAnchor','tour'+menuItem.id);
  ...
  public createHorizontalMenuItem(menu:Array<Menu>, menuItem){
      ...
      let link = this.renderer2.createElement('a');
      this.renderer2.setProperty(li, 'hidden',menuItem.isAdmin);
      this.renderer2.addClass(link, 'menu-item-link');
      this.renderer2.setAttribute(link, 'data-toggle', 'tooltip');
  ...
  public getActiveLink(menu:Array<Menu>){
      ...
      let routerLink = (url) ? url : '/stockoptions/mygrants/';  // url.substring(1, url.length);
